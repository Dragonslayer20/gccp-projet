...
<?php
    ...
    if($_SERVER['REQUEST_METHOD']=="POST"){
        ...
        $sql1 = "select * from `buyers` where title='$title'";
        $result1 = mysqli_query($conn,$sql1) or die(mysqli_error($conn));
        $data = mysqli_fetch_array($result1);
        $buyername = $data['buyername'];
       
        $sql3 = "select * from `".$buyername."buyer` where title= '$title' and buyers='$user'";
        $result3 = mysqli_query($conn,$sql3) or die(mysqli_error($conn));
        $num = mysqli_num_rows($result3);
      if($num == 0){  
            $sql="insert into `".$buyername."buyer` values(null,'$user','$date','$category',' $location1','$requirement'
            ,'$budget','$message','$title',1)";
            $result= mysqli_query($conn,$sql) or die(mysqli_error($conn));

            $sql2="insert into `".$_SESSION['username']."seller` values('$buyername','$date','$title')";
                    ...
                    <div class="middle">   
                        <form action="view.php" method="post" >
                            <h4 style="float:left;float: left;padding-left: 10%;padding-bottom: 1%;">Requirement details</h4>
                                ...
                                <div class="loc"  style="padding-bottom: 2%; padding-left: 1%;">
